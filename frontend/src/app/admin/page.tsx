...
import { DonutChart, GradientAreaChart, EnhancedHorizontalBar, StatusDonut } from '@/components/ui/charts';
        ...
        <section style={{ padding: '48px 32px 80px' }}>
          <div style={{ maxWidth: 1280, margin: '0 auto' }}>
            ...
            {chartData && (chartData.registrationsOverTime.some(d => d.count > 0) || chartData.eventsByType.length > 0) && (
                <div data-testid="admin-charts-section" style={{ display: 'flex', flexDirection: 'column', gap: 24, marginBottom: 48 }}>
                  {/* Row 1: Area chart full width */}
                  {chartData.registrationsOverTime.some(d => d.count > 0) && (
                      <motion.div
                          data-testid="registrations-chart-card"
                          initial={{ opacity: 0, y: 16 }}
                          animate={{ opacity: 1, y: 0 }}
                          transition={{ duration: 0.5, delay: 0.1 }}
                          style={{ padding: '28px 28px 20px', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', background: 'var(--white)' }}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
                          <div>
                            <h3 style={{ fontSize: 16, fontWeight: 650, color: 'var(--text)', letterSpacing: '-0.02em' }}>
                              Registrations over time
                            </h3>
                            <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>Monthly registration trend</p>
                          </div>
                          <div style={{
                            padding: '5px 12px',
                            ...
                            {chartData.registrationsOverTime.reduce((s, d) => s + d.count, 0)} total
                          </div>
                        </div>
                        <GradientAreaChart
                            data={chartData.registrationsOverTime.map((d) => ({ label: d.month, value: d.count }))}
                            color="#6366f1"
                            height={260}
                        />
                      </motion.div>
                  ...
                  {/* Row 2: Donut + Horizontal bar side by side */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }} className="admin-chart-grid">
                    {chartData.eventsByType.length > 0 && (
                        <motion.div
                            data-testid="events-by-type-card"
                            initial={{ opacity: 0, y: 16 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5, delay: 0.15 }}
                            style={{ padding: 28, borderRadius: 'var(--radius-lg)', border: '1px solid var(--border)', background: 'var(--white)' }}
                        >
                          <h3 style={{ fontSize: 16, fontWeight: 650, color: 'var(--text)', marginBottom: 4, letterSpacing: '-0.02em' }}>
                            Events by type
                          </h3>
                          <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 24 }}>Distribution across categories</p>
                          <DonutChart
                              data={chartData.eventsByType.map((d) => ({ label: d.type, value: d.count }))}
                          />
                        </motion.div>
                    )}
                    ...
                    {chartData.registrationsByEvent.length > 0 && (
                        ...
                        >
                          ...
                          <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 24 }}>Most popular events</p>
                          <EnhancedHorizontalBar
                              data={chartData.registrationsByEvent.map((d) => ({ label: d.title, value: d.count }))}
                          />
                        </motion.div>
                    )}
                  </div>
              ...
              >
                ...
                {Object.keys(statusSummary).length > 0 && (
                    <div style={{ marginTop: 28 }}>
                      <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', marginBottom: 16, letterSpacing: '-0.01em' }}>
                        Events by status
                      </h3>
                      <StatusDonut
                          data={Object.entries(statusSummary).map(([status, count]) => ({
                            status,
                            count,
